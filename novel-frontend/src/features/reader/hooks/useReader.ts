...
import type { LayoutConfig, PageDescriptor, ImageBlock } from '@engine/layout/types';
import type { CanvasDimensions } from './useCanvasResize';

/** 翻页模式预计算图片块（由 ReaderShell 设置，供 _makeImagePage 使用） */
let _pagedImageBlocks: ImageBlock[] = [];
export function setPagedImageBlocks(blocks: ImageBlock[]) { _pagedImageBlocks = blocks; }
...
export interface UseReaderOptions {
  ...
  /**
   * 初始章节 ID（从 BookDetailPage 导航 state 传入）。
   * 优先于 IndexedDB 中保存的进度使用。
   */
...
export function useReader({
  ...
  const goToChapter = useCallback(
    async (chapterId: string, targetPageIndex?: number, startCharOffset?: number) => {
      ...
      try {
        ...
        // 移除正文中的 [插图: xxx.jpg] 标记（实际图片通过 content.images 渲染）
        let cleanText = content.content.replace(/\[插图:\s*[^\]]+\]\n?/g, '').trim();

        // 纯插图章节（清洗后无正文）→ 插入占位换行，确保排版引擎有内容可处理
        if (!cleanText && content.images && content.images.length > 0) {
          cleanText = '\n';
        }

        store.setChapter(chapterId, content.title, cleanText, content.images);
        ...
        if (config) {
          ...
          // 修正总页数：排版只算了文本页，需要加上图片页
          // 使用 content.images（修正前原始值），content 不会被 layoutCurrentChapter 内部修改
          const imgCount = content.images?.length || 0;
          if (imgCount > 0) {
            // 纯插图章节 → 不计文本页
            const hasRealText = cleanText.replace(/\s/g, '').length > 0;
            const textPages = hasRealText
              ? Math.max(1, useReaderStore.getState().totalPagesInChapter)
              : 0;
            store.setTotalPages(imgCount + textPages);
          }
        } else {
          // Canvas 尺寸尚未就绪
  ...
  const openBook = useCallback(
    async (source: BookSource) => {
      ...
      try {
        ...
        // 确定起始章节：导航 state 优先 → 保存的进度 → 第一章
        const { restoreReadingProgress } = await import(
          '@/services/storage/ProgressCache'
        );
        const savedProgress = await restoreReadingProgress(metadata.bookId);

        // 导航 state 显式指定了章节 → 优先使用（用户点击了某个章节）
  ...
  /** 创建图片页 PageDescriptor（含图片数据，供翻页动画预览） */
  const _makeImagePage = useCallback(
    (idx: number, chapterId: string, totalPages: number): PageDescriptor => {
      const img = _pagedImageBlocks[idx];
      return {
        pageIndex: idx,
        chapterId,
        lines: [],
  ...
  /**
   * Turn page forward（支持图片页，保留原始按需排版逻辑）
   */
  const nextPage = useCallback(async () => {
    const state = useReaderStore.getState();
    if (!state.chapterId || !state.chapterText) return;

    const imgCount = state.chapterImages?.length || 0;
    // 仅图片章节使用简化逻辑
    if (imgCount > 0) {
      const total = state.totalPagesInChapter;
      const curIdx = state.currentPage?.pageIndex ?? 0;
      const nextIdx = curIdx + 1;
      if (nextIdx >= total) {
        const nc = state.chapterNav?.getNext(state.chapterId);
        if (nc) { await goToChapter(nc.chapterId, 0); }
        return;
      }
      const from = state.currentPage!;
      let to: PageDescriptor;
      if (nextIdx < imgCount) {
        to = _makeImagePage(nextIdx, state.chapterId, total);
      } else {
        const textIdx = nextIdx - imgCount;
        const cache = pageCacheRef.current;
        let tp = cache.get(state.chapterId, textIdx);
        if (!tp) { await ensurePagesAround(textIdx, 3); tp = cache.get(state.chapterId, textIdx); }
        if (!tp) return;
        to = { ...tp, pageIndex: nextIdx, totalPagesKnown: total };
      }
      if (onBeforePageTurn) { await onBeforePageTurn(from, to, 1); }
      store.setCurrentPage(to, from, null);
      persistProgress();
      return;
    }
    ...
    if (state.nextPage) {
      const from = state.currentPage!;
      const to = state.nextPage;
      const cache = pageCacheRef.current;
      const nextNext = cache.get(state.chapterId, to.pageIndex + 1);
      if (!nextNext && !to.isLastPage) {
        await ensurePagesAround(to.pageIndex + 1, 3);
      }
      if (onBeforePageTurn) { await onBeforePageTurn(from, to, 1); }
      const nextNextCached = cache.get(state.chapterId, to.pageIndex + 1) ?? null;
      store.setCurrentPage(to, from, nextNextCached);
      persistProgress();
      return;
    }
    ...
    if (state.currentPage) {
      ...
      const cache = pageCacheRef.current;
      const tp = cache.get(state.chapterId, state.currentPage.pageIndex + 1);
      if (tp) {
        if (onBeforePageTurn) { await onBeforePageTurn(state.currentPage, tp, 1); }
        store.setCurrentPage(tp, state.currentPage, null);
        persistProgress();
      }
    }
  }, [store, ensurePagesAround, goToChapter, onBeforePageTurn, persistProgress, _makeImagePage]);

  /**
   * Turn page backward（支持图片页，保留原始按需排版逻辑）
   */
  const prevPage = useCallback(async () => {
    ...
    if (imgCount > 0) {
      ...
      if (prevIdx < 0) {
        const pc = state.chapterNav?.getPrev(state.chapterId);
        if (pc) { await goToChapter(pc.chapterId, -1); }
        return;
      }
      const from = state.currentPage!;
      let to: PageDescriptor;
      if (prevIdx < imgCount) {
        to = _makeImagePage(prevIdx, state.chapterId, total);
      } else {
        const textIdx = prevIdx - imgCount;
        const cache = pageCacheRef.current;
        let tp = cache.get(state.chapterId, textIdx);
        if (!tp) { await ensurePagesAround(textIdx, 3); tp = cache.get(state.chapterId, textIdx); }
        if (!tp) return;
        to = { ...tp, pageIndex: prevIdx, totalPagesKnown: total };
      }
      if (onBeforePageTurn) { await onBeforePageTurn(from, to, -1); }
      store.setCurrentPage(to, null, from);
      persistProgress();
      return;
    }
    ...
    if (state.prevPage) {
      const from = state.currentPage!;
      const to = state.prevPage;
      const cache = pageCacheRef.current;
      const prevPrev = cache.get(state.chapterId, to.pageIndex - 1) ?? null;
      if (onBeforePageTurn) { await onBeforePageTurn(from, to, -1); }
      store.setCurrentPage(to, prevPrev, from);
      persistProgress();
      return;
    }
    ...
    if (state.currentPage) {
      ...
      if (tp) {
        if (onBeforePageTurn) { await onBeforePageTurn(state.currentPage, tp, -1); }
        store.setCurrentPage(tp, null, state.currentPage);
        persistProgress();
      }
    }
  }, [store, ensurePagesAround, goToChapter, onBeforePageTurn, persistProgress, _makeImagePage]);

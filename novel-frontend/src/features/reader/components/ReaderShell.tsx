...
/**
 * 为章节首页注入插图块。
 * 计算图片总高度，将文本行下移，并生成 ImageBlock 数组。
 */
function injectImagesToFirstPage(
  page: import('@engine/layout/types').PageDescriptor,
  images: ChapterImage[],
  contentWidth: number,
  pageWidth: number,
  preloadedImages: Map<string, HTMLImageElement>,
): import('@engine/layout/types').PageDescriptor {
  if (!page.isFirstPage || images.length === 0) return page;

  const leftMargin = (pageWidth - contentWidth) / 2;
  const blocks = createImageBlocks(images, preloadedImages, contentWidth, leftMargin);

  // 计算图片区总高度
  const IMG_GAP = 12;
  const totalShift = blocks.reduce((sum, b) => sum + b.height, 0)
    + (blocks.length + 1) * IMG_GAP;

  // 下移文本行
  const shiftedLines = page.lines.map((line) => ({
    ...line,
    y: line.y + totalShift,
  }));

  return { ...page, lines: shiftedLines, images: blocks };
}

interface ReaderShellProps {
  bookSource: BookSource;
  onBack: () => void;
  /** 初始章节 ID（从详情页导航 state 传入，优先于已保存进度） */
...
export const ReaderShell: React.FC<ReaderShellProps> = ({
  ...
  // ─── 章节切换检测（滚动模式）───
  /** 上一次渲染的章节 ID，用于检测章节切换时强制清空画布 */
  const lastRenderedChapterRef = useRef<string>('');

  // ─── 图片预加载 ───
  /** 预加载的图片计数器版本号——每次图片就绪时递增，触发渲染 effect 重跑 */
  const [imagesVersion, setImagesVersion] = useState(0);
  /** 已预加载的图片映射，供渲染 effect 计算图片尺寸 */
  const preloadedImagesRef = useRef<Map<string, HTMLImageElement>>(new Map());

  // 当章节内容中的图片列表变化时，预加载所有图片
  useEffect(() => {
    ...
    // 立即写入 ref：渲染 effect 可立即获取图片元素（即使未加载完成）
    ...
    images.forEach((img) => {
      ...
      el.onload = el.onerror = () => {
        ...
        if (loaded >= images.length) {
          ...
          // 图片全部就绪 → 递增版本号触发重渲染（此时已有 naturalWidth/Height）
  ...
  useEffect(() => {
    ...
    if (imgCount > 0 && curIdx < imgCount && imageBlocksRef.current.length > 0) {
      const imgPage: import('@engine/layout/types').PageDescriptor = {
        ...
        isFirstPage: curIdx === 0,
        isLastPage: curIdx >= totalPages - 1,
        totalPagesKnown: totalPages,
        images: [imageBlocksRef.current[curIdx]],
      };
  ...
  // 翻页模式：排版完成后修正总页数（图片页 + 文本页）
  // 注意：layoutCurrentChapter 会调用 setTotalPages 覆盖为文本页数，
  // 因此必须在排版完成后（layoutVersion 递增时）再次修正。
  // 滚动模式：排版完成后恢复上次滚动位置
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (chapterImages && chapterImages.length > 0) {
      ...
      scrollImages = createImageBlocks(
        chapterImages,

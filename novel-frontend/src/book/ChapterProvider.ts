...
export async function loadChapter(
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      // 检测旧缓存：如果缺少 imagesJson 或正文仍有 [插图: 标记，
      // 说明是旧版本缓存，绕过缓存重新获取
      const hasMarkers = /\[插图:/.test(cached.content);
      if (!cached.imagesJson && hasMarkers) {
        // 旧缓存 → 强制刷新（递归调用带 forceRefresh）
        console.log(
          `[ChapterProvider] 检测到旧缓存 (book=${bookId}, ch=${chapterId})，自动刷新`,
        );
      } else {
        // 有效缓存 → 反序列化图片并返回
        let images: ChapterContent['images'] = [];
        if (cached.imagesJson) {
          try {
            images = JSON.parse(cached.imagesJson);
  ...
  // 清理正文中的 [插图: xxx] 标记后再缓存
  const cleanContent = content.content.replace(/\[插图:\s*[^\]]+\]\n?/g, '');

  // Cache for future use（存储清理后的正文和图片列表）
  const cached: CachedChapter = {
    chapterId: `${bookId}:${chapterId}`,
    bookId,
    title: content.title,
    content: cleanContent,
    imagesJson: content.images.length > 0

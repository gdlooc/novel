...
import type { PageDescriptor, LayoutConfig, ImageBlock } from '../layout/types';
...
export function paintPage(
  ...
  // 2. Draw images (before text, behind text)
  if (page.images && options.preloadedImages) {
    for (const imgBlock of page.images) {
      drawImage(ctx, imgBlock, options.preloadedImages);
    }
  }
...
/**
 * Draw a preloaded image on the canvas.
 * Uses the image block's position and dimensions, fitting within the content area.
 */
function drawImage(
  ctx: CanvasRenderingContext2D,
  imgBlock: ImageBlock,
  ...
  const img = preloadedImages.get(imgBlock.url);
  ...
  // 居中绘制（x 为内容区左边距，width 为可用宽度）
  const drawWidth = Math.min(width, img.naturalWidth);
  const drawHeight = Math.min(height, img.naturalHeight);
  const scale = Math.min(drawWidth / img.naturalWidth, drawHeight / img.naturalHeight, 1);

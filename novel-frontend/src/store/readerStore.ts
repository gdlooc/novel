...
import type { BookSource, ChapterNav, BookMetadata, ChapterImage } from '@book/types';
...
export interface ReaderState {
  ...
  /** 当前章节的插图列表（用于图片预加载和渲染） */
  chapterImages: ChapterImage[] | null;
...
export const useReaderStore = create<ReaderState>()((set, get) => ({
  ...
  setChapter: (chapterId, title, text, images) => {
    set({
      chapterId,
      chapterTitle: title,
      chapterText: text,
      chapterImages: images ?? null,
      currentPageIndex: 0,
      currentCharOffset: 0,
      currentPage: null,
      nextPage: null,

...
> 生成日期: 2026-05-31
> 更新日期: 2026-06-18 (ApiAdapter 直连 PostgreSQL + 文件/API 双模式共存)
...
| API 适配器 | `book/formats/ApiAdapter.ts` | HTTP API 适配器，直连 FastAPI → PostgreSQL | ✅ |
...
│   │   │   ├── ApiAdapter.ts          # HTTP API 适配器（PostgreSQL）
│   │   │   ├── WenkuAdapter.ts       # 爬虫输出 JSON 适配器（文件）
